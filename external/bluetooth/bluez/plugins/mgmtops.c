...
#ifdef BOARD_HAVE_BLUETOOTH_STE
#include "ste-qos.h"
#endif
...
#ifdef BOARD_HAVE_BLUETOOTH_STE
static void mgmt_flow_specification_complete(int sk, uint16_t index, void *buf,
								size_t len)
{
	struct mgmt_rp_flow_spec *rp = buf;
	char addr[18];

	if (len != sizeof(*rp)) {
		error("Wrong flow_specification_complete event size");
		return;
	}

	if (rp->status) {
		error("flow specification failed with reason %d", rp->status);
		return;
	}

	if (index > max_index) {
		error("Unexpected index %u in flow_specification_complete", index);
		return;
	}

	ba2str(&rp->bdaddr, addr);

	DBG("hci%u flow_specification_for %s", index, addr);

	DBG("%d token bucket size %d token rate %d access latency "
		"%d peak bandwidth  %d direction %d service type %d flags",
		rp->token_bucket_size, rp->token_rate, rp->access_latency,
		rp->peak_bandwidth, rp->direction, rp->service_type, rp->flags);
}

static void mgmt_vs_ext_flow_specification_complete(int sk, uint16_t index,
						 void *buf, size_t len)
{
	struct mgmt_rp_vs_flow_spec *rp = buf;
	char addr[18];

	if (len != sizeof(*rp)) {
		error("Wrong vs_ext_flow_specification_complete event size");
		return;
	}

	if (rp->status) {
		error("vs_ext_flow_specification failed with reason %d", rp->status);
		return;
	}

	if (index > max_index) {
		error("Unexpected index %u in "
				"vs_ext_flow_specification_complete", index);
		return;
	}

	ba2str(&rp->bdaddr, addr);

	DBG("hci%u vs_ext_flow_specification_for %s flow_interval %d flow window %d",
				 index, addr, rp->interval, rp->window);
}
#endif
...
static void mgmt_cmd_complete(int sk, uint16_t index, void *buf, size_t len)
{
	...
	case MGMT_OP_FLOW_SPECIFICATION:
		mgmt_vs_ext_flow_specification_complete(sk, index, ev->data, len);
		break;
	case MGMT_OP_FORBID_ROLE_SWITCH:
		DBG("forbid_role_switch complete");
		break;
#endif
	default:
...
// SSBT :: KJH + to check encryption error
	...
	case MGMT_EV_REMOTE_FEATURES:
		...
		break;
#ifdef BOARD_HAVE_BLUETOOTH_STE
	case MGMT_EV_FLOW_SPECIFICATION_COMPLETE:
		mgmt_flow_specification_complete(sk, index, buf + MGMT_HDR_SIZE, len);
		break;
#endif
	default:
...
#ifdef BOARD_HAVE_BLUETOOTH_STE
static int mgmt_set_qos(int index, bdaddr_t *bdaddr,
					struct ste_qos_params *params)
{
	char buf[MGMT_HDR_SIZE + sizeof(struct mgmt_cp_flow_spec)];
	struct mgmt_hdr *hdr = (void *) buf;
	struct mgmt_cp_flow_spec *cp = (void *) &buf[sizeof(*hdr)];
	char addr[18];
	...
	hdr->opcode = htobs(MGMT_OP_FLOW_SPECIFICATION);
	hdr->index = htobs(index);
	hdr->len = htobs(sizeof(*cp));

	bacpy(&cp->bdaddr, bdaddr);

	cp->service_interval = params->service_interval;
	cp->out_service_window = params->out_service_window;
	cp->in_service_window = params->in_service_window;
	cp->cqae = params->cqae;
	cp->packet_size = params->packet_size;

	cp->token_bucket_size = params->token_bucket_size;
	cp->token_rate = params->token_rate;
	cp->access_latency = params->access_latency;
	cp->peak_bandwidth = params->peak_bandwidth;
	cp->direction = params->direction;
	cp->service_type = params->service_type;
	cp->flags = params->flags;
	DBG("Service Interval = 0x%04x out_service_window = 0x%04x "
		"in_service_window = 0x%04x "
		"cqae = 0x%04x packet_size = 0x%04x",
		cp->service_interval,
		cp->out_service_window,
		cp->in_service_window,
		cp->cqae,
		cp->packet_size);
...
static int mgmt_forbid_role_switch(int index, bdaddr_t *bdaddr)
{
	char buf[MGMT_HDR_SIZE + sizeof(struct mgmt_cp_forbid_role_switch)];
	struct mgmt_hdr *hdr = (void *) buf;
	struct mgmt_cp_forbid_role_switch *cp = (void *) &buf[sizeof(*hdr)];
	char addr[18];
	...
	hdr->opcode = htobs(MGMT_OP_FORBID_ROLE_SWITCH);
	hdr->index = htobs(index);
	hdr->len = htobs(sizeof(*cp));
...
#ifdef BOARD_HAVE_BLUETOOTH_STE
	.set_qos = mgmt_set_qos,
	.forbid_role_switch = mgmt_forbid_role_switch,
#endif
};
